...
def generate_caption(model_path, image_path, vocab, max_len=20, device='cuda'):
    ...
    # Encode the image using the encoder part of the model
    with torch.no_grad():
    ...
    # Convert the predicted caption to a string
...
def generate_captions_for_csv(csv_path, image_dir, model_path, vocab, output_csv_path, max_len=20, device='cuda'):
    ...
    df['predicted_captions'] = 'place holder'
    ...
    # Create a transform to preprocess images
    transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
